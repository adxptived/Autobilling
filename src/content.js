    ...
    if (isPaymentFrame) {
      ...
      if (inputs.length > 0) {
        for (var j = 0; j < inputs.length; j++) {
          ...
          var inpInfo = identifyInputByContext(inp);
          if (inpInfo) {
...
function identifyInputByContext(inp) {
  var id = (inp.id || '').toLowerCase();
  var name = (inp.name || '').toLowerCase();
  var ph = (inp.placeholder || '').toLowerCase();
  var aria = (inp.getAttribute('aria-label') || '').toLowerCase();
  ...
  if (/card.?number|cc.?number|ccnum|pan/i.test(combined)) return { key: 'cardNumber', value: '' };
  if (/expir|cc.?exp|valid/i.test(combined)) return { key: 'expiry', value: '' };
  if (/cvv|cvc|csc|cid|security.?code/i.test(combined)) return { key: 'cvv', value: '' };
  if (isNum && maxLen >= 13 && maxLen <= 19) return { key: 'cardNumber', value: '' };
  if (isNum && maxLen >= 3 && maxLen <= 4) return { key: 'cvv', value: '' };
  return null;
}
